...
\chapter{Segmentation sémantique multimodale}
...
\chapsummary{%
\lettrine{L}{es} chapitres précédents nous ont permis d'établir de nouveaux états de l'art en segmentation sémantique d'images de télédétection sur l'ensemble des capteurs optiques couramment mis en \oe{}uvre pour l'observation de la Terre. Cependant, nous avons également observé l'insuffisance des modèles numériques de terrain dérivés du \gls{Lidar} lorsqu'ils sont exploités seuls.
...
\section{Apprentissage multimodal}
\label{sec:multimodal}

\subsection{Réseaux de neurones et apprentissage multimodal}
...
\begin{table}[h]
    \caption{Résultats de segmentation sémantique multimodale sur le jeu de validation \glsname{ISPRS} Vaihingen.}
...
\begin{table}[h]
    \mycaption{Résultats de segmentation sémantique multimodale sur le jeu de test \glsname{ISPRS} Vaihingen (approches multimodales).}{Les meilleurs résultats sont en \textbf{gras} et les seconds sont en \emph{italique}.}
...
\begin{table}[h]
    \mycaption{Résultats de segmentation sémantique multimodale sur le jeu de test \glsname{ISPRS} Potsdam (approches multimodales).}{Les meilleurs résultats sont en \textbf{gras} et les seconds sont en \emph{italique}.}
    \label{table:final_multimodal_potsdam}
...
\begin{figure}[hb]
	\begin{subfigure}{\textwidth}
        ...
        \begin{subfigure}[t]{0.19\textwidth}
        	...
        	\caption*{SegNet-CR}
        \end{subfigure}
        \caption{Prédictions de différents modèles sur un extrait du jeu de données \glsname{ISPRS} Vaihingen.}
	...
	\mycaption{Exemples de prédictions multimodales réussies sur Vaihingen.}{\\\isprslegende}
...
\begin{figure}[h]
    ...
    \caption{V-FuseNet}
    \end{subfigure}
    \mycaption{Effet des stratégies de fusion sur un extrait du jeu de données \glsname{ISPRS} Potsdam.}{La confusion entre les classes de route et de bâtiments est nettement réduite grâce à la contribution des modèles de terrain.\\\isprslegende}
...
\begin{figure}[h]
    ...
    \begin{subfigure}{0.24\textwidth}
        ...
        \caption{V-FuseNet}
    \end{subfigure}
	\mycaption{Les erreurs dans le \glsname{MNH} du jeu de données \glsname{ISPRS} Vaihingen sont mal gérées par les deux méthodes de fusion.}{Ici, un bâtiment entier disparaît.}
    \label{fig:ndsm_fail}
\end{figure}
...
\begin{figure}[h]
  ...
  \mycaption{Tuile 4\_12 du jeu de données \glsname{ISPRS} Potsdam et données \glsname{OSM} correspondantes.}{\\\isprslegende}
...
\begin{table}[t]
	\mycaption{Résultats de segmentation sémantique multimodale avec \glsname{OSM} sur le jeu de données \glsname{ISPRS} Potsdam}{(scores $F_1$ par classe et pourcentage global de pixels bien classés).}
...
\end{subfigure}
\mycaption{Exemple de segmentation obtenue sur le jeu de données ISPRS Potsdam en incluant la donnée \glsname{OSM}.}{Les erreurs sur les bâtiments sont grandement réduites.\\\isprslegende}
...
\end{tabularx}
\caption{Évolution des prédictions de SegNet \glssymbol{RVB} et \glssymbol{RVB}+\glsname{OSM}.}{L'ajout de \glsname{OSM} rend les prédictions visuellement plus structurées.\\\isprslegende}
...
\publication{Les travaux présentés dans la~\cref{sec:multimodal} ont été le sujet d'un article de journal:}{audebert_beyond_2017}
\publication{Les travaux présentés dans la~\cref{sec:joint} ont été le sujet d'une publication en conférence:}{audebert_joint_2017}

...
\todo[inline]{En fait, la fusion tardive est forte pour combiner des prédictions complémentaires quand les deux modèles sont sûrs d'eux (par exemple, le parking sur le toit : le composite est sûr que c'est un bâtiment grâce au Lidar, l'optique est sûr que ce sont des voitures. FuseNet va plutôt être fort pour apprendre des représentations conjointes, mais tombe dans le même travers initial que SegNet : les voitures sur le toit disparaissent car c'est un cas hyper rare qui n'arrive que dans quelques occurrences (et normalement, une voiture c'est sur la route...).}
...
\subsection{Robustness to uncertainties and missing data}

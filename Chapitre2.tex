...
\chapsummary{%
	...
	La classification par région consiste à partitionner l'image en régions homogènes via un algorithme de segmentation non-supervisé. Les régions ainsi obtenues sont ensuites classifiées une à une. Nous étudions plus particulièrement l'utilisation de caractéristiques profondes utilisant des \gls{CNN} pré-entraînés. Nous montrons notamment que les caractéristiques profondes de réseaux entraînés sur des images multimédia peuvent se transférer avec succès pour l'analyse d'images aériennes.

	Par ailleurs, nous mettons en avant le rôle limitant des segmentations non-supervisées dans le processus de classification par région. Par une analyse détaillée, nous montrons que ces segmentations sont nécessairement imparfaites et sous-résolues, et que diminuer leur influence revient à effectuer une classification pixel à pixel extrêmement coûteuse. Nous étudions ainsi des approches par réseaux de neurones entièrement convolutifs, capables de réaliser une extraction de caractéristiques et une classification dense sur tous les pixels d'une image en une seule inférence.

	Nous adaptons plusieurs modèles de l'état de l'art pour la segmentation sémantique d'images naturelles au traitement de données de télédétection, sur lesquelles nous montrons la supériorité empirique des tels réseaux entièrement convolutifs par rapport aux méthodes de classification usuelles.
}
...
\begin{definition}
La classification par région d'une image $\mathcal{I}$ consiste à trouver une partition $P = {P_1, \dots, P_n}$ telle que\,:
$$\wbigcup_{i=1}^n P_i = \mathcal{I}~~~\text{(segmentation)}$$
...
\begin{figure}
	\begin{subfigure}{0.48\textwidth}
		\includegraphics[width=\textwidth]{multiscale_ds_segnet.pdf}
        \caption{Inférence multi-échelles sur un modèle SegNet.}
    \end{subfigure}
    \begin{subfigure}{0.48\textwidth}
		\includegraphics[width=\textwidth]{multiscale_ds_segnet_back.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{contextual_module}
...
\begin{table}
	...
	\begin{tabularx}{\textwidth}{Y c c c c c c}
    \toprule
    Nombre de branches & routes & bâtiments & vég. basse & arbres & véhicules & Globale\\
    \midrule
